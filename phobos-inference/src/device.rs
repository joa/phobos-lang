...
use phobos_gguf::compute::{
    Attn, Backend, Buf, DeltaMix, Plane, Q8_BLOCK, QAct, QBuf, Rope, check_q8_shape,
};
...
/// cust keeps its own status conversion private, and the graph API is only
/// reachable through the raw bindings.
fn cuda_ok(status: cust::sys::CUresult, what: &str) -> Result<()> {
    match status {
...
pub(crate) const ATTN_GEMM_TILE: usize = 64;
...
pub(crate) const ATTN_SOFT_TILE: usize = 32;
...
pub(crate) fn attn_gemm_src(head_dim: usize, tile: usize) -> String {
...
pub struct DeviceBackend {
    ...
    /// Released allocations by exact element count, handed out again rather
    /// than going back to the driver. See [`Backend::alloc`].
    pool: RefCell<HashMap<usize, Vec<DeviceBuffer<f32>>>>,
...
impl DeviceBackend {
    pub fn new() -> Result<DeviceBackend> {
        ...
        let matmul = compile_variants(
            MATMUL_SRC,
            &[("TILE_M", TILE_M), ("TILE_N", TILE_N), ("TILE_K", TILE_K)],
            "matmul",
            ("@aligned(M = TILE_M, N = TILE_N)", ""),
        )?;
        let matvec = compile_variants(
            MATVEC_SRC,
        ...
        let q8_dp4a = compile_variants(
            Q8_DP4A_SRC,
            &[("TN", Q8_TN)],
            "q8_dp4a",
            ("@aligned(N = TN, K = 32)", "@aligned(K = 32)"),
        )?;
        let q8_mma = compile_variants(
            Q8_MMA_SRC,
    ...
    ///
    /// Ordinary allocations come from the pool, which keeps a decode step's
    /// addresses stable across steps. But the pool's free list is only free in
    /// the sense that no handle points at it: while a pass is recording, a
    /// buffer released earlier in that same pass is still read by launches
    /// recorded and not yet run. An immediate write lands before those launches
    /// do, so a pooled buffer would overwrite their input. A fresh allocation
    /// cannot collide, since the pool holds every released buffer alive.
    ///
    ...
    fn alloc_written_now(&self, len: usize) -> Result<Buf> {
        ...
        // SAFETY: the caller writes every element before anything reads it,
        // which is the point of asking for this rather than `alloc`.
        Ok(self.store(unsafe { DeviceBuffer::uninitialized(len)? }))
    }
...
/// A kernel compiled both ways. `aligned` claims the grid tiles its output
/// evenly, which keeps the register-blocked and dp4a fast paths; `general`
/// claims nothing and masks the boundary tile. The launch picks by the shape it
/// has, so a ragged one costs only itself.
struct Variants {
    aligned: Module,
    general: Module,
}
...
impl Variants {
    fn pick(&self, tiles_evenly: bool) -> &Module {
        if tiles_evenly {
            &self.aligned
        } else {
            &self.general
...
/// Compiles both variants of a source carrying an `{ALIGNED}` placeholder.
/// `claims` holds the aligned variant's claim and the general variant's. They
/// are separate because only one `@aligned` attribute is read per kernel, so a
/// promise the caller keeps whatever the grid looks like has to appear in
/// both.
fn compile_variants(
    source: &str,
    shapes: &[(&str, usize)],
...
fn compile(source: &str, shapes: &[(&str, usize)], what: &str) -> Result<Module> {
    Ok(compile_shared(source, shapes, what)?.0)
}

/// Compiles, also reporting what each `@dynshared` kernel needs at launch.
/// Static shared memory caps at 48 KB whatever the card has, since a static
/// buffer is a declared array. A kernel asking for its tiles dynamically gets
/// whatever the launch passes, and past 48 KB the driver wants the function's
...
impl Backend for DeviceBackend {
    fn alloc(&self, len: usize) -> Result<Buf> {
        // An exact-size buffer from the pool when one is free.
        //
        // Reuse used to corrupt the model while every operation still matched
        // the host reference in isolation. The kernels were writing past their
        // output: a matvec rounded up to a whole 128-element tile, so a 16-wide
        // projection wrote 112 floats into whatever came next. Fresh
        // allocations hid it in slack and reuse put a live buffer there. The
        // bounds masks fixed the write.
        if let Some(buffer) = self.pool.borrow_mut().get_mut(&len).and_then(Vec::pop) {
            return Ok(self.store(buffer));
        }
        // SAFETY: every operation writes its destination before it is read,
        // and the model never reads an allocation it has not filled.
        Ok(self.store(unsafe { DeviceBuffer::uninitialized(len)? }))
    }
