...
/// pairing is the loader's, not the format's; see [`BpeTokenizer::load`].
...
impl OnnxModel {
    /// Load whichever export `dir` holds, with the tokenizer files beside it.
    pub fn load(dir: &Path) -> Result<OnnxModel> {
        OnnxModel::load_with_tokenizer(dir, dir)
    }
    ...
    pub fn load_with_tokenizer(dir: &Path, tokenizer_dir: &Path) -> Result<OnnxModel> {
        ...
        let (graph, vocab_size, engine_name) = match &engine {
            Engine::Full(g) => (&g.graph, g.vocab, "full-recompute"),
            Engine::Kv(g) => (&g.prompt_graph, g.vocab, "KV-cached"),
        };
        let context_limit = position_embedding_len(graph).unwrap_or(DEFAULT_CONTEXT);
        Ok(OnnxModel {
            info: ModelInfo {
...
fn position_embedding_len(graph: &Graph) -> Option<usize> {
    graph
        .initializers
        .iter()
        .find(|(name, _)| name.ends_with("wpe.weight") || name.contains("position_embeddings"))
        .and_then(|(_, t)| t.dims.first())
