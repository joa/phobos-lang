...
use phobos_kernels::util::{pad, round_up};
...
const MM_TM: usize = matmul::TILE_M;
const MM_TN: usize = matmul::TILE_N;
const MM_TK: usize = matmul::TILE_K;
...
pub struct ChainExec {
    ...
    /// still alive. Declared first, teardown faulted after a clean run, which
    /// is why `chain_gpt2` used to print its result and then exit 0xc0000005.
...
impl ChainExec {
    pub fn new() -> Result<Self> {
        ...
        let matmul = phobos_kernels::compile(&matmul::src(), &matmul::shapes(), "matmul")?;
        Ok(ChainExec {
            stream,
            kernels: HashMap::new(),
            slots: HashMap::new(),
    ...
    pub fn run(
        ...
        for (name, t) in host::decode_initializers(graph)? {
    ...
    fn exec_host(&mut self, node: &Node) -> Result<()> {
        ...
        let outs = host::run_node_host(node, &ins)?;
...
/// Compile a plan's Phobos source, pinning its autotune tile dims.
fn compile(plan: &lower::KernelPlan) -> Result<Module> {
    ...
    Ok(phobos_kernels::compile_in(&ctx, &plan.source, &plan.kernel_name)?.0)
}
