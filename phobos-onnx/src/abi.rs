/// One kernel argument, width-tagged so the launcher can lay each out in its
/// own 8-byte little-endian slot for `cuLaunchKernel`.
...
impl KernelArg {
    /// The argument's bytes in the low end of an 8-byte slot. x86 is
    /// little-endian, so a kernel param expecting 4 bytes reads the right value
    /// from a pointer to this slot.
...
/// Append the flattened memref descriptor for a row-major tensor of `dims`
/// backed by `ptr`: allocated pointer, aligned pointer, offset, per-axis sizes,
/// per-axis strides.
...
/// Row-major strides for `dims`: the last axis has stride 1, and each earlier
/// axis multiplies in the extent of the axes to its right.
