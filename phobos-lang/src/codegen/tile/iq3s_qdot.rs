...
impl<'c> Codegen<'c> {
    ...
    pub(in crate::codegen) fn tile_iq3s_qdot_t(
        ...
        if grid.elem != self.i8_t || signs.elem != self.i8_t {
            bail!("iq3s_qdot_t's grid and sign tables must hold packed i8 lanes");
        }
        ...
        let geom = self.iq3s_lane(&body, lane)?;
