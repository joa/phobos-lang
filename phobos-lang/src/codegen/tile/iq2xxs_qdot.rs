...
impl<'c> Codegen<'c> {
    ...
    pub(in crate::codegen) fn tile_iq2xxs_qdot_t(
        ...
        if grid.elem != self.i8_t || signs.elem != self.i8_t {
            bail!("iq2xxs_qdot_t's grid and sign tables must hold packed i8 lanes");
        }
        ...
        let geom = self.iq2xxs_lane(&body, lane)?;
        ...
        let dec = self.iq2xxs_block(&kb, &geom, qb, d, &QTables { grid, signs }, &at)?;

        let mut partial = carry;
        let k_off = self.addi(&kb, kbase, geom.k_lane_off)?;
        let a_t = Type::vector(&[ACT_VEC as u64], f32_t);
        for g in 0..IQ2XXS_LANE / ACT_VEC {
            let goff = self.const_index(&kb, g * ACT_VEC)?;
            let a_at = self.addi(&kb, k_off, goff)?;
            let a_v = self.vec_load(&kb, a.mem, &[zero_idx, a_at], a_t)?;
            for i in 0..ACT_VEC {
                let decoded = self.iq2xxs_decoded(&kb, &dec, g * ACT_VEC + i)?;
