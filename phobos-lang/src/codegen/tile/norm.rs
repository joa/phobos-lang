...
impl<'c> Codegen<'c> {
    pub(in crate::codegen) fn emit_rms_norm_q(&mut self, block: &Block<'c>, args: &[Expr]) -> Result<Rv<'c>> {
        ...
        for i in 0..per {
            let off = c(self, i * stripe)?;
            let e = self.addi(block, e0, off)?;
            let row = self.divui(block, e, thirty_two)?;
            let col = self.remui(block, e, thirty_two)?;
            let present = match (i + 1) * stripe > width {
                true => Some(self.push(
                    block,
                    arith::cmpi(self.ctx, arith::CmpiPredicate::Ult, e, width_idx, self.loc),
                )?),
                false => None,
            };
            at.push((row, col, present));
        }
        ...
        for (row, col, present) in &at {
            ...
            if let Some(o) = &o {
                self.vec_store_al(dst, out, o.mem, &[*row, *col], 16)?;
            }
            self.vec_store_al(dst, packed, q.mem, &[*row, *col], 4)?;
            let store = Block::new(&[]);
            store.append_operation(memref::store(scale, s.mem, &[*row, zero], self.loc));
            store.append_operation(scf::r#yield(&[], self.loc));
            let sr = Region::new();
            sr.append_block(store);
            dst.append_operation(scf::r#if(leads_block, &[], sr, Region::new(), self.loc));
            if let (Some(present), Some(gated)) = (present, gated) {
                gated.append_operation(scf::r#yield(&[], self.loc));
                let region = Region::new();
                region.append_block(gated);
                block.append_operation(scf::r#if(*present, &[], region, Region::new(), self.loc));
            }
        }
        for t in [&x, &g, &q, &s] {
            self.release(t);
    ...
    fn load4_present(
        ...
        mem: Value<'c, 'c>,
        indices: &[Value<'c, 'c>],
        present: Option<Value<'c, 'c>>,
    ) -> Result<Value<'c, 'c>> {
        let vec4 = Type::vector(&[4], self.f32_t);
        let Some(present) = present else {
            return self.vec_load_al(block, mem, indices, vec4, 16);
        };
        let then = Block::new(&[]);
        let v = self.vec_load_al(&then, mem, indices, vec4, 16)?;
        then.append_operation(scf::r#yield(&[v], self.loc));
        let els = Block::new(&[]);
        let zero = self.zero_scalar(&els, self.f32_t)?;
        let zeros = self.vec_broadcast(&els, zero, vec4)?;
        els.append_operation(scf::r#yield(&[zeros], self.loc));
        let (tr, er) = (Region::new(), Region::new());
        tr.append_block(then);
        er.append_block(els);
        self.push(block, scf::r#if(present, &[vec4], tr, er, self.loc))
    }
}
