// Fused IQ1_M dot: grid-table decode folded into the contraction. The decode
// itself lives in `iq1m.rs`, shared with `qdecode.rs`.
...
impl<'c> Codegen<'c> {
    ...
    pub(in crate::codegen) fn tile_iq1m_qdot_t(
        ...
        if grid.elem != self.i8_t {
            bail!("iq1m_qdot_t's grid table must hold packed i8 lanes");
        }
        ...
        let geom = self.iq1m_lane(&body, lane)?;
        ...
        for g in 0..IQ1M_LANE / ACT_VEC {
            ...
            for i in 0..ACT_VEC {
                let decoded = self.iq1m_decoded(&kb, &dec, g * ACT_VEC + i)?;
