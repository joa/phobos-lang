...
impl<'c> Codegen<'c> {
    ...
    pub(in crate::codegen) fn tile_iq3xxs_qdot_t(
        ...
        if grid.elem != self.i8_t || signs.elem != self.i8_t {
            bail!("iq3xxs_qdot_t's grid and sign tables must hold packed i8 lanes");
        }
        ...
        let geom = self.iq3xxs_lane(&body, lane)?;
        let zero_idx = geom.zero_idx;
        ...
        let dec = self.iq3xxs_block(&kb, &geom, qb, d, &QTables { grid, signs }, &at)?;

        let mut partial = carry;
        let k_off = self.addi(&kb, kbase, geom.k_lane_off)?;
        let zero_idx_kb = self.const_index(&kb, 0)?;
        let a_t = Type::vector(&[IQ3XXS_HALF as u64], f32_t);
        for entry in 0..2i64 {
            let half_off = self.const_index(&kb, entry * IQ3XXS_HALF)?;
            let a_at = self.addi(&kb, k_off, half_off)?;
            let a_v = self.vec_load(&kb, a.mem, &[zero_idx_kb, a_at], a_t)?;
            for y in 0..IQ3XXS_HALF {
                let decoded = self.iq3xxs_decoded(&kb, &dec, entry, y)?;
                let a_val = self.vec_extract(&kb, a_v, &[y], f32_t)?;
