// `iq2xs_qdot_t`: see phobos-lang/src/codegen/tile/iq2xs_qdot.rs.
...
kernel iq2xs_qdot_matvec(A: tensor<f32>[M, K], QB: tensor<i8>[N, RB],
                         D: tensor<f16>[N, NB], GRID: tensor<i32>[1, 4096],
...
#[test]
fn iq2xs_qdot_t_has_no_per_lane_staging() {
...
#[test]
fn iq2xs_qdot_t_reads_a_dynamic_contraction_length_at_runtime() {
    let mlir = emit_mlir(SRC);
    assert!(
...
#[test]
fn iq2xs_qdot_t_rejects_a_ragged_contraction() {
    let src = "@launch(256)
        kernel iq2xs_decode(A: tensor<f32>[1, 128], QB: tensor<i8>[8, 37],
                            D: tensor<f16>[8, 1], GRID: tensor<i32>[1, 4096],
                            SIGNS: tensor<i32>[1, 1024], C: tensor<f32>[1, 8]) {
            C[0 :+ 1, 0 :+ 8] = iq2xs_qdot_t(A[0 :+ 1, :], QB[0 :+ 8, :], D[0 :+ 8, :], GRID[0 :+ 1, :], SIGNS[0 :+ 1, :])
        }";
