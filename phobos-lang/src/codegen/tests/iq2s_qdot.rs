// `iq2s_qdot_t`: see phobos-lang/src/codegen/tile/iq2s_qdot.rs.
...
kernel iq2s_qdot_matvec(A: tensor<f32>[M, K], QB: tensor<i8>[N, RB],
                        D: tensor<f16>[N, NB], GRID: tensor<i8>[1, 8192],
                        SIGNS: tensor<i8>[1, 2048], C: tensor<f32>[M, N]) {
  let pn = program_id(0)
...
#[test]
fn iq2s_qdot_t_has_no_per_lane_staging() {
...
#[test]
fn iq2s_qdot_t_reads_a_dynamic_contraction_length_at_runtime() {
    let mlir = emit_mlir(SRC);
    assert!(
...
#[test]
fn iq2s_qdot_t_rejects_a_ragged_contraction() {
    let src = "@launch(256)
        kernel iq2s_decode(A: tensor<f32>[1, 128], QB: tensor<i8>[8, 41],
                           D: tensor<f16>[8, 1], GRID: tensor<i8>[1, 8192],
                           SIGNS: tensor<i8>[1, 2048], C: tensor<f32>[1, 8]) {
            C[0 :+ 1, 0 :+ 8] = iq2s_qdot_t(A[0 :+ 1, :], QB[0 :+ 8, :], D[0 :+ 8, :], GRID[0 :+ 1, :], SIGNS[0 :+ 1, :])
        }";
