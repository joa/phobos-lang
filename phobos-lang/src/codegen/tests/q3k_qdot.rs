// `q3k_qdot_t`: Q3_K's matvec contraction with its static-offset decode
// folded in. See `phobos-lang/src/codegen/tile/q3k_qdot.rs`.
...
#[test]
fn q3k_qdot_t_has_no_per_lane_staging() {
...
#[test]
fn q3k_qdot_t_reads_a_dynamic_contraction_length_at_runtime() {
    let mlir = emit_mlir(SRC);
    assert!(
...
#[test]
fn q3k_qdot_t_rejects_a_ragged_contraction() {
    let src = "@launch(256)
        kernel q3k_decode(A: tensor<f32>[1, 128], QB: tensor<i8>[32, 55],
                          D: tensor<f16>[32, 1], C: tensor<f32>[1, 32]) {
            C[0 :+ 1, 0 :+ 32] = q3k_qdot_t(A[0 :+ 1, :], QB[0 :+ 32, :], D[0 :+ 32, :])
        }";
