...
- **K-quant contraction**: `q4k`, `q5k` and `q6k` join `<fmt>_qgemm_t` and
  ...
  are, and a run's dot product stays under 2^22, so the exact add-and-subtract
  conversion applies. Q4_K and Q5_K subtract a minimum, `d * sc * q - dmin *
  m`, so against an activation `sa * aq` the contraction is
  `sa * (d * sc * A - dmin * m * S)` with `S` the activation's own sum over the
  ...
  `<fmt>_qdot_i8_t` sums the row once in a prologue into a shared tile of 1024
  groups, which bounds `K` at 32768. Both read `d` and `dmin` out of the block
  header for Q4_K and Q5_K, so the `d` operand is taken and never read; Q6_K's
  `d` trails its block on disk, is dropped from the device block (208 bytes of
  210), and is read from the plane. Q6_K has no minimum: its `q - 32` folds into
  the byte before the `dp4a`, exactly and without a carry, as
  `ql | (((qh2 + 0x0E0E0E0E) & 0x0F0F0F0F) << 4)`, and its scales are per
