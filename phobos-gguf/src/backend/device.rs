...
use phobos_kernels::launch::{CTA_THREADS, STATIC_SHARED_LIMIT, WARP_THREADS, persistent_grid};
...
/// Pieces the key axis is cut into while decoding. Fixed rather than chosen from
/// the cache length, which is what it wants to be: a count that grows with the
/// cache reshapes the pass every few dozen tokens, and each change costs a graph
/// rebuild. A piece with no keys only costs a block that exits immediately.
